...
Production intentionally refuses to start with the `sa` login, disabled SQL encryption, `TrustServerCertificate=True`, placeholder values, wildcard hosts, a non-HTTPS public origin, or relative storage/key paths. Use a separate, temporary migration identity with schema permissions; never place those elevated credentials in the App Service web configuration. If TLS terminates at a proxy, set `ForwardedHeaders__KnownProxies__0` (and additional indexed values as needed) only to that proxy's trusted IP address.
